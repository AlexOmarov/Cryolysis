...
-- Send user message
------------------------------------------------------------------------------------------------------
function CRYOLYSIS:SendUserMessage(msg)
    ChatFrame1:AddMessage(CRYOLYSIS_USER_MESSAGE_INTRO .. colorize(msg), 0.2, 0.9, 0.95, 1.0, UIERRORS_HOLD_TIME);
end

function CRYOLYSIS:SendWorldMessage(msg)
...
function CRYOLYSIS:SendGroupMessage(msg, type)
    if (GetNumRaidMembers() > 0 and type == "GROUP") then
        SendChatMessage(msg, "RAID");
    elseif (GetNumPartyMembers() > 0 and type == "GROUP") then
        SendChatMessage(msg, "PARTY");
    else SendChatMessage(msg, type);
    end
end

function CRYOLYSIS:ShowError(msg)
    UIErrorsFrame:AddMessage(CRYOLYSIS_USER_MESSAGE_INTRO .. colorize(msg), 0.2, 0.9, 0.95, 1.0, UIERRORS_HOLD_TIME);
end
