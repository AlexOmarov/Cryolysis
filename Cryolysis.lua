...
Default_CryolysisConfig = {
    Version = CryolysisData.Version; -- The version from attrs.lua
    ...
    HPLimit = 85; -- Eat food when under self amount
    MPLimit = 100; -- Drink water when under self amount
    ...
    SpellTimerPos = 1;
    SpellTimerJust = "LEFT";
    Food = 1; -- I dont remember what self is
    Graphical = true;
    Yellow = true;
    SensListe = 1;
...
if CryolysisConfig.CryolysisLanguage == "zhTW" then
    PortalName = {
        "奧格瑪", "幽暗城", "雷霆崖", "鐵爐堡", "暴風城", "達納�?�斯", -- 1-6, Teleports
        "奧格瑪", "幽暗城", "雷霆崖", "鐵爐堡", "暴風城", "達納�?�斯" -- 7-12, Portals
    }
end
...
function Cryolysis_OnLoad()
    ...
    CryolysisButton:RegisterEvent("PLAYER_ENTERING_WORLD");
    CryolysisButton:RegisterEvent("PLAYER_LEAVING_WORLD");
    CryolysisButton:RegisterEvent("BAG_UPDATE");
    CryolysisButton:RegisterEvent("COMBAT_LOG_EVENT_UNFILTERED");
    --[[CryolysisButton:RegisterEvent("CHAT_MSG_SPELL_PERIODIC_SELF_BUFFS");]]
    --[[CryolysisButton:RegisterEvent("COMBAT_LOG_EVENT_AURA_REMOVED");
	CryolysisButton:RegisterEvent("COMBAT_LOG_EVENT_AURA_REMOVED");
	CryolysisButton:RegisterEvent("COMBAT_LOG_EVENT_AURA_BROKEN");]]
...
function Cryolysis_LoadVariables()
    ...
    if Cryolysis_Loaded or englishClass ~= "MAGE" then
        return
    end
...
function Cryolysis_OnUpdate()
    ...
    if (not Cryolysis_Loaded) and englishClass ~= "MAGE" then
        return;
    end
    ...
    if curTime >= debuff.drReset and debuff.drPlayer then
        ...
        debuff.drReset = 0;
        SpellCastName = L["CRYOLYSIS_SPELL_TABLE"][67].Name;
                    ...
                    if SpellTimer[index] then
                        ...
                        if (update) then
                            ...
                            if SpellTimer and (SpellTimer[index].Type == 4 or SpellTimer[index].Type == 5)
                                    and SpellTimer[index].Target == UnitName("target") then
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][49].ID ~= nil then
        start, duration = GetSpellCooldown(L["CRYOLYSIS_SPELL_TABLE"][49].ID, BOOKTYPE_SPELL);
    else
        start = 1;
        duration = 1;
    end
...
function Cryolysis_OnEvent(event)
    ...
    elseif event == "COMBAT_LOG_EVENT" then -- WINTERSCHILL will go here
        local timestamp, type, hideCaster, sourceGUID, sourceName, sourceFlags, sourceFlags2, destGUID, destName, destFlags, destFlags2, twelve, auraType = CombatLogGetCurrentEventInfo()
        -- Note, for self example, you could just use 'local type = select(2, ...)'.  The others are included so that it's clear what's available.
        if type == "SPELL_PERIODIC" then
            for creatureName, spell in string.gfind(arg1, CRYOLYSIS_DEBUFF_SRCH) do
                -- Frostbite
                if spell == L["CRYOLYSIS_SPELL_TABLE"][53].Name then
                    SpellCastName = spell;
                    SpellTargetName = creatureName;
                    Cryolysis_SpellManagement();
                    -- Winter's Chill
                elseif spell == L["CRYOLYSIS_SPELL_TABLE"][54].Name
                        or spell == L["CRYOLYSIS_SPELL_TABLE"][55].Name
                        or spell == L["CRYOLYSIS_SPELL_TABLE"][56].Name
                        or spell == L["CRYOLYSIS_SPELL_TABLE"][57].Name
                        or spell == L["CRYOLYSIS_SPELL_TABLE"][58].Name then
                    for selftimer = table.getn(SpellTimer), 1, -1 do
                        if SpellTimer[selftimer].Name == L["CRYOLYSIS_SPELL_TABLE"][54].Name
                                or SpellTimer[selftimer].Name == L["CRYOLYSIS_SPELL_TABLE"][55].Name
                                or SpellTimer[selftimer].Name == L["CRYOLYSIS_SPELL_TABLE"][56].Name
                                or SpellTimer[selftimer].Name == L["CRYOLYSIS_SPELL_TABLE"][57].Name
                                or SpellTimer[selftimer].Name == L["CRYOLYSIS_SPELL_TABLE"][58].Name then
                            SpellTimer, TimerTable = Cryolysis_RetraitTimerParIndex(selftimer, SpellTimer, TimerTable);
                        end
                    end
                    SpellCastName = spell;
                    SpellTargetName = creatureName;
                    Cryolysis_SpellManagement();
                    -- Fire Vulnerability
                elseif spell == L["CRYOLYSIS_SPELL_TABLE"][59].Name
                        or spell == L["CRYOLYSIS_SPELL_TABLE"][60].Name
                        or spell == L["CRYOLYSIS_SPELL_TABLE"][61].Name
                        or spell == L["CRYOLYSIS_SPELL_TABLE"][62].Name
                        or spell == L["CRYOLYSIS_SPELL_TABLE"][63].Name then
                    for selftimer = table.getn(SpellTimer), 1, -1 do
                        if SpellTimer[selftimer].Name == L["CRYOLYSIS_SPELL_TABLE"][59].Name
                                or SpellTimer[selftimer].Name == L["CRYOLYSIS_SPELL_TABLE"][60].Name
                                or SpellTimer[selftimer].Name == L["CRYOLYSIS_SPELL_TABLE"][61].Name
                                or SpellTimer[selftimer].Name == L["CRYOLYSIS_SPELL_TABLE"][62].Name
                                or SpellTimer[selftimer].Name == L["CRYOLYSIS_SPELL_TABLE"][63].Name then
                            SpellTimer, TimerTable = Cryolysis_RetraitTimerParIndex(selftimer, SpellTimer, TimerTable);
                        end
                    end
                    SpellCastName = spell;
                    ...
                    --			elseif spell == L["CRYOLYSIS_SPELL_TABLE"][65].Name then
                    --				SpellCastName = spell;
                    --				SpellTargetName = creatureName;
                    --				SpellTargetLevel = "";
                    --				Cryolysis_ChatMessage(SpellCastName, SpellTargetName);
                    --				Cryolysis_SpellManagement();
                    -- Frost Nova
                elseif spell == L["CRYOLYSIS_SPELL_TABLE"][19].Name then
                    SpellCastName = L["CRYOLYSIS_SPELL_TABLE"][66].Name
                    SpellTargetName = L["CRYOLYSIS_SPELL_TABLE"][19].Name;
                    SpellTargetLevel = "";
                    Cryolysis_SpellManagement();
                end
            end
            -- Peronsal actions -- "Buffs"
        elseif (type == "SPELL_PERIODIC_AURA_APPLIED" and auraType == "BUFF") then
            Cryolysis_SelfEffect("BUFF");
            -- Personal actions -- "Debuffs"
        elseif (type == "SPELL_PERIODIC_AURA_REMOVED" or event == "SPELL_PERIODIC_AURA_BROKEN") and auraType == "DEBUFF" then
            Cryolysis_SelfEffect("DEBUFF");
        elseif type == "SPELL_PERIODIC_AURA_REMOVED" then
...
function Cryolysis_RegisterManagement(RegistrationType)
    if RegistrationType == "IN" then
        ...
        --[[CryolysisButton:RegisterEvent("SPELL_PERIODIC_COMBAT_LOG_EVENT");
		CryolysisButton:RegisterEvent("COMBAT_LOG_EVENT_AURA_REMOVED");
		CryolysisButton:RegisterEvent("COMBAT_LOG_EVENT_AURA_REMOVED");
		CryolysisButton:RegisterEvent("COMBAT_LOG_EVENT_AURA_BROKEN");]]
        CryolysisButton:RegisterEvent("PLAYER_REGEN_DISABLED");
        ...
        CryolysisButton:UnregisterEvent("COMBAT_LOG_EVENT_UNFILTERED");
        --[[CryolysisButton:UnregisterEvent("SPELL_PERIODIC_COMBAT_LOG_EVENT");
		CryolysisButton:UnregisterEvent("COMBAT_LOG_EVENT_AURA_REMOVED");
		CryolysisButton:UnregisterEvent("COMBAT_LOG_EVENT_AURA_REMOVED");
		CryolysisButton:UnregisterEvent("COMBAT_LOG_EVENT_AURA_BROKEN");]]
...
function Cryolysis_SelfEffect(action)
    ...
    if action == "BUFF" then
        ...
        if string.find(arg1, L["CRYOLYSIS_SPELL_TABLE"][49].Name) and L["CRYOLYSIS_SPELL_TABLE"][49].ID ~= nil then
            EvocationUp = false;
            CryolysisEvocationButton:SetNormalTexture("Interface\\Addons\\Cryolysis\\UI\\Evocation-03");
        end
        if string.find(arg1, L["CRYOLYSIS_SPELL_TABLE"][23].Name) and L["CRYOLYSIS_SPELL_TABLE"][15].ID ~= nil then
            IceBarrierUp = false;
            CryolysisBuffMenu4:SetNormalTexture("Interface\\Addons\\Cryolysis\\UI\\IceBarrier-03");
        end
        if string.find(arg1, L["CRYOLYSIS_SPELL_TABLE"][15].Name) and L["CRYOLYSIS_SPELL_TABLE"][15].ID ~= nil then
            FireWardUp = false;
            CryolysisBuffMenu5:SetNormalTexture("Interface\\Addons\\Cryolysis\\UI\\FireWard-03");
        end
        if string.find(arg1, L["CRYOLYSIS_SPELL_TABLE"][20].Name) and L["CRYOLYSIS_SPELL_TABLE"][20].ID ~= nil then
            FireWardUp = false;
            CryolysisBuffMenu5:SetNormalTexture("Interface\\Addons\\Cryolysis\\UI\\FireWard-03");
        end
    else
        if string.find(arg1, L["CRYOLYSIS_SPELL_TABLE"][43].Name) then
            SpellCastName = L["CRYOLYSIS_SPELL_TABLE"][43].Name
            CombustionFade = true;
            Cryolysis_SpellManagement()
        end
        if string.find(arg1, L["CRYOLYSIS_SPELL_TABLE"][44].Name) then
            SpellCastName = L["CRYOLYSIS_SPELL_TABLE"][44].Name
            PoMFade = true;
            Cryolysis_SpellManagement()
        end
...
function Cryolysis_SpellManagement()
    ...
    if (SpellCastName) then
        ...
        if CryolysisConfig.ShowSpellTimers then
            if SpellCastName == L["CRYOLYSIS_SPELL_TABLE"][42].Name and L["CRYOLYSIS_SPELL_TABLE"][42].ID ~= nil then
                for selftimer = table.getn(SpellTimer), 1, -1 do
                    if SpellTimer[selftimer].Name == L["CRYOLYSIS_SPELL_TABLE"][9].Name
                            or SpellTimer[selftimer].Name == L["CRYOLYSIS_SPELL_TABLE"][19].Name
                            or SpellTimer[selftimer].Name == L["CRYOLYSIS_SPELL_TABLE"][20].Name
                            or SpellTimer[selftimer].Name == L["CRYOLYSIS_SPELL_TABLE"][23].Name
                            or SpellTimer[selftimer].Name == L["CRYOLYSIS_SPELL_TABLE"][41].Name then
                        SpellTimer, TimerTable = Cryolysis_RetraitTimerParIndex(selftimer, SpellTimer, TimerTable);
                    end
                end
            end
            --		-- Pour les autres sorts cast�s, tentative de timer si valable
            for spell = 1, table.getn(L["CRYOLYSIS_SPELL_TABLE"]), 1 do
                if SpellCastName == L["CRYOLYSIS_SPELL_TABLE"][spell].Name then -- and not (spell == 10) then  <--- and the spell isn't Enslave Demon
                    -- If a timer already exists, it is updated
                    for selfspell = 1, table.getn(SpellTimer), 1 do
                        if SpellTimer[selfspell].Name == SpellCastName
                                and SpellTimer[selfspell].Target == SpellTargetName
                                and SpellTimer[selfspell].TargetLevel == SpellTargetLevel
                                and L["CRYOLYSIS_SPELL_TABLE"][spell].Type ~= 4 then
                            ...
                            SpellTimer[selfspell].Time = L["CRYOLYSIS_SPELL_TABLE"][spell].Length;
                            SpellTimer[selfspell].TimeMax = floor(GetTime() + L["CRYOLYSIS_SPELL_TABLE"][spell].Length);
                            -- adjusts the duration for polymorph based on the rank
                            if spell == 26
                                    or spell == 48
                                    or spell == 52 then
                                if SpellCastRank == nil then SpellCastRank = L["CRYOLYSIS_SPELL_TABLE"][26].Rank; end
                                SpellTimer[selfspell].Time, SpellTimer[selfspell].TimeMax = Cryolysis_PvPPoly(SpellTargetName);
                            end
                            SortActif = true;
                            break;
                        end
                        ...
                        if SpellTimer[selfspell].Name == SpellCastName
                                ...
                                or spell == 52
                                and
                                (SpellTimer[selfspell].Target ~= SpellTargetName
                                        or SpellTimer[selfspell].TargetLevel ~= SpellTargetLevel) then
                            SpellTimer, TimerTable = Cryolysis_RetraitTimerParIndex(selfspell, SpellTimer, TimerTable);
                            SortActif = false;
                            break;
                        end
                        if SortActif then break; end
                    end
                    if not SortActif and L["CRYOLYSIS_SPELL_TABLE"][spell].Type ~= 0 then
                        if spell == 26
                                or spell == 48
                                or spell == 52 then
                            L["CRYOLYSIS_SPELL_TABLE"][spell].Length = Cryolysis_PvPPoly(SpellTargetName);
                        end
...
function Cryolysis_PolyCheck(type, spell, creatureName)
    ...
    elseif type == "stop" then
        if CryolysisConfig.PolyWarn and spell == L["CRYOLYSIS_SPELL_TABLE"][26].Name
                or spell == L["CRYOLYSIS_SPELL_TABLE"][48].Name
                or spell == L["CRYOLYSIS_SPELL_TABLE"][52].Name then
            if SpellCastRank == nil then
                SpellCastRank = L["CRYOLYSIS_SPELL_TABLE"][26].Rank
            end
    ...
    elseif type == "break" and creatureName == CryolysisPrivate.PolyTarget then
        if spell == L["CRYOLYSIS_SPELL_TABLE"][26].Name then
            ...
            for selftimer = table.getn(SpellTimer), 1, -1 do
                if SpellTimer[selftimer].Name == L["CRYOLYSIS_SPELL_TABLE"][26].Name
                        and SpellTimer[selftimer].Target == creatureName then
                    SpellTimer, TimerTable = Cryolysis_RetraitTimerParIndex(selftimer, SpellTimer, TimerTable);
                    break;
                end
            end
        ...
        if spell == L["CRYOLYSIS_SPELL_TABLE"][48].Name then
            if CryolysisConfig.Sound and CryolysisConfig.PolyBreak and CryolysisPrivate.PolyTarget == creatureName then
                PlaySoundFile(CRYOLYSIS_SOUND.PigBreak);
                CryolysisPrivate.PolyTarget = nil;
            end
            for selftimer = table.getn(SpellTimer), 1, -1 do
                if SpellTimer[selftimer].Name == L["CRYOLYSIS_SPELL_TABLE"][48].Name
                        and SpellTimer[selftimer].Target == creatureName then
                    SpellTimer, TimerTable = Cryolysis_RetraitTimerParIndex(selftimer, SpellTimer, TimerTable);
                    break;
                end
...
function Cryolysis_ChatMessage(spell, creatureName)
    if CryolysisPrivate.ChatSilence or not CryolysisConfig.ChatMsg then
        ...
        -- Polymorph
        if spell == L["CRYOLYSIS_SPELL_TABLE"][26].Name
                or spell == L["CRYOLYSIS_SPELL_TABLE"][48].Name
                or spell == L["CRYOLYSIS_SPELL_TABLE"][52].Name then
                ...
                if CryolysisPrivate.PolyTarget ~= creatureName then
                    if not CryolysisConfig.SM then
                        ...
                        if spell == L["CRYOLYSIS_SPELL_TABLE"][26].Name then
                            ...
                            -- Pig
                        elseif spell == L["CRYOLYSIS_SPELL_TABLE"][48].Name then
                            ...
                            -- Turtle
                        elseif spell == L["CRYOLYSIS_SPELL_TABLE"][52].Name then
        ...
        elseif spell == L["CRYOLYSIS_SPELL_TABLE"][65].Name then
            ...
            local port;
            for i = 1, 12, 1 do
                if spell == L["CRYOLYSIS_SPELL_TABLE"][PortalTempID[i]].Name then
                    port = i;
                    break;
                end
            end
...
function Cryolysis_MerchantCheck()
    ...
    for item = 1, MerchItems do
        ...
        if itemString == L["CRYOLYSIS_ITEM"].ArcanePowder
                or itemString == L["CRYOLYSIS_ITEM"].RuneOfTeleportation
                or itemString == L["CRYOLYSIS_ITEM"].RuneOfPortals then
            Cryolysis_BagCheck("Force");
            -- Check For teleports
            for i = 1, 6, 1 do
                if L["CRYOLYSIS_SPELL_TABLE"][PortalTempID[i]].ID then
                    display = true;
            ...
            -- Check for powder
            if L["CRYOLYSIS_SPELL_TABLE"][2].ID then
                display = true;
...
function Cryolysis_Restock()
    ...
    local RestockCount = {};
    local RestockNames = {
        [1] = L["CRYOLYSIS_ITEM"].RuneOfTeleportation;
        [2] = L["CRYOLYSIS_ITEM"].RuneOfPortals;
        [3] = L["CRYOLYSIS_ITEM"].ArcanePowder;
    };
    for i = 1, 6, 1 do
        if L["CRYOLYSIS_SPELL_TABLE"][PortalTempID[i]].ID ~= nil then
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][2].ID ~= nil then
    ...
    for item = 1, MerchItems do
        for i = 1, table.getn(RestockCount) do
            ...
            if itemString == RestockNames[i] and RestockCount[i] > 0 then
                Cryolysis_Msg(L["CRYOLYSIS_MESSAGE"].Information.Restock .. RestockCount[i] .. " " .. RestockNames[i], "USER");
...
function Cryolysis_Toggle(button, keybind)
    ...
    if button == "LeftButton" then
        if CryolysisConfig.Button == 1 then
            ...
            local MPPercent = floor((UnitPower("player") or 1) / (UnitPowerMax("player") or 1) * 100)
            if HPPercent < CryolysisConfig.HPLimit then
                if Count.Food > 0 then
                    ...
                    Cryolysis_Msg(L["CRYOLYSIS_MESSAGE"].Error.NoFood, "USER");
                end
            end
            if MPPercent < CryolysisConfig.MPLimit then
                if Count.Drink > 0 then
                    ...
                    Cryolysis_Msg(L["CRYOLYSIS_MESSAGE"].Error.NoDrink, "USER");
                end
            end
            Cryolysis_BagCheck("Update");
        ...
        elseif CryolysisConfig.Button == 5 then
            CastSpell(L["CRYOLYSIS_SPELL_TABLE"][7].ID, "spell");
...
function Cryolysis_BuildTooltip(button, type, anchor)
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][49].ID then
        start, duration = GetSpellCooldown(L["CRYOLYSIS_SPELL_TABLE"][49].ID, BOOKTYPE_SPELL);
    else
        start = 1;
        duration = 1;
    end
    if L["CRYOLYSIS_SPELL_TABLE"][15].ID then
        start2, duration2 = GetSpellCooldown(L["CRYOLYSIS_SPELL_TABLE"][15].ID, BOOKTYPE_SPELL);
    else
        start2 = 1;
        duration2 = 1;
    end

    if L["CRYOLYSIS_SPELL_TABLE"][20].ID then
        start3, duration3 = GetSpellCooldown(L["CRYOLYSIS_SPELL_TABLE"][20].ID, BOOKTYPE_SPELL);
    else
        start3 = 1;
        duration3 = 1;
    end
    if L["CRYOLYSIS_SPELL_TABLE"][23].ID then
        start4, duration4 = GetSpellCooldown(L["CRYOLYSIS_SPELL_TABLE"][23].ID, BOOKTYPE_SPELL);
    else
        start4 = 1;
        duration4 = 1;
    end
    ...
    GameTooltip:SetText(L["CryolysisTooltipData"][type].Label);
    -- ..... For the main Sphere

    if (type == "Main") then
        GameTooltip:AddLine(L["CryolysisTooltipData"].Main.Food .. Count.Food);
        GameTooltip:AddLine(L["CryolysisTooltipData"].Main.Drink .. Count.Drink);
        GameTooltip:AddLine(L["CryolysisTooltipData"].Main.RuneOfTeleportation .. Count.RuneOfTeleportation);
        GameTooltip:AddLine(L["CryolysisTooltipData"].Main.RuneOfPortals .. Count.RuneOfPortals);
        GameTooltip:AddLine(L["CryolysisTooltipData"].Main.ArcanePowder .. Count.ArcanePowder);
        GameTooltip:AddLine(L["CryolysisTooltipData"].Main.LightFeather .. Count.LightFeather);
    elseif (type == "Mount") then
        GameTooltip:SetText(L["CryolysisTooltipData"][type].Label .. Mount.Title .. "|r");
        ...
        if string.find(itemName, L["CRYOLYSIS_TRANSLATION"].Cooldown) then
            GameTooltip:AddLine(L["CRYOLYSIS_TRANSLATION"].Hearth .. " - " .. itemName);
        else
            GameTooltip:AddLine(L["CryolysisTooltipData"]["SpellTimer"].Right .. GetBindLocation());
        end
        -- Manastone
    elseif (type == "Manastone") then
        local ManastoneTooltip = {
            CRYOLYSIS_STONE_RANK2[1] .. L["CryolysisTooltipData"].Manastone.Text[4],
            CRYOLYSIS_STONE_RANK2[2] .. L["CryolysisTooltipData"].Manastone.Text[4],
            CRYOLYSIS_STONE_RANK2[3] .. L["CryolysisTooltipData"].Manastone.Text[4],
            CRYOLYSIS_STONE_RANK2[4] .. L["CryolysisTooltipData"].Manastone.Text[4],
        };
        ...
        for i = StoneMaxRank[2], 1, -1 do
            if i == StoneMaxRank[2] and Manastone.Mode[i] == 1 then
                ManastoneTooltip[i] = CRYOLYSIS_STONE_RANK2[i] .. L["CryolysisTooltipData"].Manastone.Text[1] .. Manastone.MP[i] .. " Mana"
            ...
            if CryolysisConfig.ManaStoneOrder == 2 then
                ...
                if Manastone.Mode[i] == 2 and i ~= ManaStoneNext then
                    ManastoneTooltip[i] = CRYOLYSIS_STONE_RANK2[i] .. L["CryolysisTooltipData"].Manastone.Text[3];
                elseif i == ManaStoneNext and Manastone.Mode[i] == 2 then
                    ManastoneTooltip[i] = CRYOLYSIS_STONE_RANK2[i] .. L["CryolysisTooltipData"].Manastone.Text[2] .. Manastone.Restore[i];
                end
            elseif CryolysisConfig.ManaStoneOrder == 1 then
                ...
                if Manastone.Mode[i] == 2 and i ~= ManaStoneNext then
                    ManastoneTooltip[i] = CRYOLYSIS_STONE_RANK2[i] .. L["CryolysisTooltipData"][type].Text[3];
                elseif i == ManaStoneNext and Manastone.Mode[i] == 2 then
                    ManastoneTooltip[i] = CRYOLYSIS_STONE_RANK2[i] .. L["CryolysisTooltipData"][type].Text[2] .. Manastone.Restore[i];
                end
            end
        end
        for i = StoneMaxRank[2], 1, -1 do
    ...
    elseif (type == "SpellTimer") then
        ...
        CryolysisTooltip:SetBagItem(HearthstoneLocation[1], HearthstoneLocation[2]);
        local itemName = tostring(CryolysisTooltipTextLeft5:GetText());
        GameTooltip:AddLine(L["CryolysisTooltipData"][type].Text);
        if string.find(itemName, L["CRYOLYSIS_TRANSLATION"].Cooldown) then
            GameTooltip:AddLine(L["CRYOLYSIS_TRANSLATION"].Hearth .. " - " .. itemName);
        else
            GameTooltip:AddLine(L["CryolysisTooltipData"][type].Right .. GetBindLocation());
        end

        -- ..... For the Concentration button
    elseif (type == "Armor") then
        if L["CRYOLYSIS_SPELL_TABLE"][22].ID then
            GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][22].Mana .. " Mana");
        else
            GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][18].Mana .. " Mana");
        end
        if L["CRYOLYSIS_SPELL_TABLE"][24].ID ~= nil then
            GameTooltip:AddLine(L["CryolysisTooltipData"].Alt.Left .. L["CRYOLYSIS_SPELL_TABLE"][24].Name .. " (" .. L["CRYOLYSIS_SPELL_TABLE"][24].Mana .. " Mana)" .. L["CryolysisTooltipData"].Alt.Right);
        end
    elseif (type == "FireWard") then
        GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][15].Mana .. " Mana");
        ...
        if L["CRYOLYSIS_SPELL_TABLE"][20].ID ~= nil then
            GameTooltip:AddLine(L["CryolysisTooltipData"].Alt.Left .. L["CRYOLYSIS_SPELL_TABLE"][20].Name .. " (" .. L["CRYOLYSIS_SPELL_TABLE"][20].Mana .. " Mana)" .. L["CryolysisTooltipData"].Alt.Right);
        end
    elseif (type == "IceBarrier") then
        GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][23].Mana .. " Mana");
        ...
        if L["CRYOLYSIS_SPELL_TABLE"][25].ID ~= nil then
            GameTooltip:AddLine(L["CryolysisTooltipData"].Alt.Left .. L["CRYOLYSIS_SPELL_TABLE"][25].Name .. " (" .. L["CRYOLYSIS_SPELL_TABLE"][25].Mana .. " Mana)" .. L["CryolysisTooltipData"].Alt.Right);
        end
    elseif (type == "ManaShield") then
        GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][25].Mana .. " Mana");
    elseif (type == "DetectMagic") then
        GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][50].Mana .. " Mana");
    elseif (type == "T:Org") then
        GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][38].Mana .. " Mana");
        GameTooltip:AddLine(L["CryolysisTooltipData"].Main.RuneOfTeleportation .. Count.RuneOfTeleportation);
    elseif (type == "T:UC") then
        GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][40].Mana .. " Mana");
        GameTooltip:AddLine(L["CryolysisTooltipData"].Main.RuneOfTeleportation .. Count.RuneOfTeleportation);
    elseif (type == "T:TB") then
        GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][39].Mana .. " Mana");
        GameTooltip:AddLine(L["CryolysisTooltipData"].Main.RuneOfTeleportation .. Count.RuneOfTeleportation);
    elseif (type == "T:IF") then
        GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][37].Mana .. " Mana");
        GameTooltip:AddLine(L["CryolysisTooltipData"].Main.RuneOfTeleportation .. Count.RuneOfTeleportation);
    elseif (type == "T:SW") then
        GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][51].Mana .. " Mana");
        GameTooltip:AddLine(L["CryolysisTooltipData"].Main.RuneOfTeleportation .. Count.RuneOfTeleportation);
    elseif (type == "T:Darn") then
        GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][36].Mana .. " Mana");
        GameTooltip:AddLine(L["CryolysisTooltipData"].Main.RuneOfTeleportation .. Count.RuneOfTeleportation);
    elseif (type == "P:Org") then
        GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][47].Mana .. " Mana");
        GameTooltip:AddLine(L["CryolysisTooltipData"].Main.RuneOfPortals .. Count.RuneOfPortals);
    elseif (type == "P:UC") then
        GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][31].Mana .. " Mana");
        GameTooltip:AddLine(L["CryolysisTooltipData"].Main.RuneOfPortals .. Count.RuneOfPortals);
    elseif (type == "P:TB") then
        GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][30].Mana .. " Mana");
        GameTooltip:AddLine(L["CryolysisTooltipData"].Main.RuneOfPortals .. Count.RuneOfPortals);
    elseif (type == "P:IF") then
        GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][28].Mana .. " Mana");
        GameTooltip:AddLine(L["CryolysisTooltipData"].Main.RuneOfPortals .. Count.RuneOfPortals);
    elseif (type == "P:SW") then
        GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][29].Mana .. " Mana");
        GameTooltip:AddLine(L["CryolysisTooltipData"].Main.RuneOfPortals .. Count.RuneOfPortals);
    elseif (type == "P:Darn") then
        GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][27].Mana .. " Mana");
        GameTooltip:AddLine(L["CryolysisTooltipData"].Main.RuneOfPortals .. Count.RuneOfPortals);
    elseif (type == "DampenMagic") then
        GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][13].Mana .. " Mana");
        if L["CRYOLYSIS_SPELL_TABLE"][1].ID ~= nil then
            GameTooltip:AddLine(L["CryolysisTooltipData"].Alt.Left .. L["CRYOLYSIS_SPELL_TABLE"][1].Name .. " (" .. L["CRYOLYSIS_SPELL_TABLE"][1].Mana .. " Mana)" .. L["CryolysisTooltipData"].Alt.Right);
        end
    elseif (type == "ArcaneInt") then
        GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][4].Mana .. " Mana");
        if L["CRYOLYSIS_SPELL_TABLE"][2].ID ~= nil then
            GameTooltip:AddLine(L["CryolysisTooltipData"].Alt.Left .. L["CRYOLYSIS_SPELL_TABLE"][2].Name .. " (" .. L["CRYOLYSIS_SPELL_TABLE"][2].Mana .. " Mana)" .. L["CryolysisTooltipData"].Alt.Right);
        end
    elseif (type == "Food") then
        GameTooltip:AddLine(L["CryolysisTooltipData"][type].Right .. " (" .. L["CRYOLYSIS_SPELL_TABLE"][10].Mana .. " Mana)");
        GameTooltip:AddLine(L["CryolysisTooltipData"][type].Middle);
        if CryolysisConfig.CryolysisLanguage == "frFR" then
            GameTooltip:AddLine(L["CRYOLYSIS_FOOD_RANK"][StoneMaxRank[4]] .. L["CRYOLYSIS_ITEM"].Provision .. ": " .. Count.Food);
        else
            GameTooltip:AddLine(L["CRYOLYSIS_ITEM"].Provision .. L["CRYOLYSIS_FOOD_RANK"][StoneMaxRank[4]] .. ": " .. Count.Food);
        end
    elseif (type == "Drink") then
        GameTooltip:AddLine(L["CryolysisTooltipData"][type].Right .. " (" .. L["CRYOLYSIS_SPELL_TABLE"][11].Mana .. " Mana)");
        GameTooltip:AddLine(L["CryolysisTooltipData"][type].Middle);
        if CryolysisConfig.CryolysisLanguage == "frFR" then
            GameTooltip:AddLine(L["CRYOLYSIS_DRINK_RANK"][StoneMaxRank[3]] .. L["CRYOLYSIS_ITEM"].Provision .. ": " .. Count.Drink);
        else
            GameTooltip:AddLine(L["CRYOLYSIS_ITEM"].Provision .. L["CRYOLYSIS_DRINK_RANK"][StoneMaxRank[3]] .. ": " .. Count.Drink);
        end
    elseif (type == "SlowFall") then
        GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][35].Mana .. " Mana");
        GameTooltip:AddLine(L["CryolysisTooltipData"].Main.LightFeather .. Count.LightFeather);
    elseif (type == "RemoveCurse") then
        GameTooltip:AddLine(L["CRYOLYSIS_SPELL_TABLE"][33].Mana .. " Mana");
    elseif (type == "Evocation") then
        if start > 0 and duration > 0 and EvocationUp == false then
            ...
            EvocationUp = true;
            GameTooltip:AddLine(L["CryolysisTooltipData"].Evocation.Text);
        end

    elseif (type == "Buff") and CryolysisPrivate.LastBuff ~= 0 then
        GameTooltip:AddLine(L["CryolysisTooltipData"].LastSpell.Left .. L["CRYOLYSIS_SPELL_TABLE"][CryolysisPrivate.LastBuff].Name .. L["CryolysisTooltipData"].LastSpell.Right);
    elseif (type == "Portal") and CryolysisPrivate.LastPortal ~= 0 then
        GameTooltip:AddLine(L["CryolysisTooltipData"].LastSpell.Left .. L["CRYOLYSIS_SPELL_TABLE"][PortalTempID[CryolysisPrivate.LastPortal]].Name .. L["CryolysisTooltipData"].LastSpell.Right);
    end
    -- And done, Showing!
    GameTooltip:Show();
end
...
function Cryolysis_BuildSpellTooltip(button, side, anchor)
    ...
    if spell == 1 then
        if L["CRYOLYSIS_SPELL_TABLE"][18].ID or L["CRYOLYSIS_SPELL_TABLE"][22].ID then
            type = "Armor";
        end
    elseif spell == 2 then
        if L["CRYOLYSIS_SPELL_TABLE"][4].ID then
            type = "ArcaneInt";
        end
    elseif spell == 3 then
        if L["CRYOLYSIS_SPELL_TABLE"][13].ID then
            type = "DampenMagic";
        end
    elseif spell == 4 then
        if L["CRYOLYSIS_SPELL_TABLE"][23].ID or L["CRYOLYSIS_SPELL_TABLE"][25].ID then
            type = "Shield";
    ...
    elseif spell == 6 then
        if L["CRYOLYSIS_SPELL_TABLE"][50].ID then
            type = "DetectMagic";
        end
    elseif spell == 7 then
        if L["CRYOLYSIS_SPELL_TABLE"][33].ID then
            type = "RemoveCurse";
        end
    elseif spell == 8 then
        if L["CRYOLYSIS_SPELL_TABLE"][35].ID then
            type = "SlowFall";
        end
    end
...
function Cryolysis_UpdateIcons()
    ...
    elseif CryolysisConfig.CountType == 5 then -- MP Current
        Sphere.display = tostring(UnitPower("player"));
    elseif CryolysisConfig.CountType == 6 then -- MP Percent
        Sphere.display = floor(UnitPower("player") / UnitPowerMax("player") * 100);
    ...
    elseif CryolysisConfig.Circle == 2 then
        Sphere.skin = "Bleu";
        Sphere.texture = floor(UnitPower("player") / (UnitPowerMax("player") / 16));
    ...
    -----------------------------------------------
    if L["CRYOLYSIS_SPELL_TABLE"][49].ID ~= nil then
        local start, duration = GetSpellCooldown(L["CRYOLYSIS_SPELL_TABLE"][49].ID, BOOKTYPE_SPELL)
    ...
    if StoneIDInSpellTable[4] ~= 0 then
        ...
        if L["CRYOLYSIS_SPELL_TABLE"][10].Mana > mana then -- No Mana
    ...
    if StoneIDInSpellTable[3] ~= 0 then
        ...
        if L["CRYOLYSIS_SPELL_TABLE"][11].Mana > mana then -- No Mana
    ...
    if StoneMaxRank[2] >= 1 then
        ...
        if CryolysisConfig.ManaStoneOrder == 1 then
            Manastone.currentStone = 1; -- Since we us mana agate first in self method, it is set as default icon
            for i = StoneMaxRank[2], 1, -1 do
                if Manastone.OnHand[i] then
                    Manastone.currentStone = i;
        ...
        -- Check to see if we have enough MP to conjure whatever stone is next.  self is displayed on the ring
    ...
    for button = 1, 3, 2 do
        for spell = 1, table.getn(CryolysisSpellButtons), 1 do
            if spellNumber[button] == spell then
                ...
                alternative = false;
                texture = 1;
                if L["CRYOLYSIS_SPELL_TABLE"][CryolysisSpellButtons[spell].ID].ID ~= nil then
                    spellEnable = true;
                    if L["CRYOLYSIS_SPELL_TABLE"][CryolysisSpellButtons[spell].ID].Mana > mana then
                        texture = 3
                    end
                elseif spell == 4 and L["CRYOLYSIS_SPELL_TABLE"][CryolysisSpellButtons[spell].ID].ID == nil then
                    if L["CRYOLYSIS_SPELL_TABLE"][25].ID ~= nil then
                        spellEnable = true;
                        alternative = true;
                        if L["CRYOLYSIS_SPELL_TABLE"][25].Mana > mana then
                            texture = 3
                        end
                    end
                end
...
function UpdatePortalMenuIcons()
    ...
    -- ManaPortal variables goes teleports then portals, listed in self order:
    -- Teleports
    ...
    for i = 1, 12, 1 do
        if L["CRYOLYSIS_SPELL_TABLE"][PortalTempID[i]].ID then
            if L["CRYOLYSIS_SPELL_TABLE"][PortalTempID[i]].Mana > mana or PlayerCombat then
                ManaPortal[i] = 3;
            end
        end
    end
    ...
    for i = 1, 12, 1 do
        if CryolysisButtonTexture.Portalmenu.Base[i] ~= ManaPortal[i] then
            local texture = _G["CryolysisPortalMenu" .. i]:GetNormalTexture():GetTexture()
            texture = string.sub(texture, 1, string.len(texture) - 1) .. ManaPortal[i];
            _G["CryolysisPortalMenu" .. i]:SetNormalTexture(texture);
...
function UpdateBuffMenuIcons()
    ...
    for i = 1, 8, 1 do
        if L["CRYOLYSIS_SPELL_TABLE"][ManaID[i]].ID then
            if L["CRYOLYSIS_SPELL_TABLE"][ManaID[i]].Mana > mana then
                ManaSpell[i] = 3;
            end
        end
    end
    if L["CRYOLYSIS_SPELL_TABLE"][23].ID ~= nil then
        local start, duration = GetSpellCooldown(L["CRYOLYSIS_SPELL_TABLE"][23].ID, "spell");
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][15].ID then
        local start, duration = GetSpellCooldown(L["CRYOLYSIS_SPELL_TABLE"][15].ID, "spell");
    ...
    for i = 1, 8, 1 do
        if CryolysisButtonTexture.Buffmenu.Base[i] ~= ManaSpell[i] then
            local texture = _G["CryolysisBuffMenu" .. i]:GetNormalTexture():GetTexture()
            texture = string.sub(texture, 1, string.len(texture) - 1) .. ManaSpell[i];
            _G["CryolysisBuffMenu" .. i]:SetNormalTexture(texture);
...
Reorder.Table = {} -- numerically indexed table of anything (just a texture in self case)
...
function Cryolysis_BagCheck(spell)
    ...
    elseif spell then
        for i = 1, table.getn(spellcheck), 1 do
            if spell == L["CRYOLYSIS_SPELL_TABLE"][spellcheck[i]].Name then
                Cryolysis_BagCheck("Update");
                break;
            end
        end
        for i = 1, table.getn(L["CRYOLYSIS_MANASTONE_NAMES"]), 1 do
            if spell == L["CRYOLYSIS_MANASTONE_NAMES"][i] then
                Cryolysis_BagCheck("Update");
                break;
            end
        end
    ...
    if not check and StoneMaxRank[2] ~= 0 then
        for i = 1, StoneMaxRank[2], 1 do
            if Manastone.OnHand[i] then
                ...
                itemName = tostring(CryolysisTooltipTextLeft1:GetText());
                if not string.find(itemName, L["CRYOLYSIS_ITEM"].Manastone .. CRYOLYSIS_STONE_RANK[i]) then
                    check = true;
                end
            end
        end
...
function Cryolysis_BagExplore()
        ...
        for slot = 1, GetContainerNumSlots(container), 1 do
            ...
            if StoneMaxRank[4] ~= 0 then
                if (CryolysisConfig.CryolysisLanguage == "zhCN") or (CryolysisConfig.CryolysisLanguage == "zhTW") then
                    FoodName = strsub(L["CRYOLYSIS_FOOD_RANK"][StoneMaxRank[4]], 2, strlen(L["CRYOLYSIS_FOOD_RANK"][StoneMaxRank[4]]));
                elseif CryolysisConfig.CryolysisLanguage == "frFR" then
                    FoodName = L["CRYOLYSIS_FOOD_RANK"][StoneMaxRank[4]] .. L["CRYOLYSIS_ITEM"].Provision;
                else
                    FoodName = L["CRYOLYSIS_ITEM"].Provision .. L["CRYOLYSIS_FOOD_RANK"][StoneMaxRank[4]];
                end
                if Count.FoodLastName == "none" then Count.FoodLastName = FoodName; end
            end
            if StoneMaxRank[3] ~= 0 then
                if (CryolysisConfig.CryolysisLanguage == "zhCN") or (CryolysisConfig.CryolysisLanguage == "zhTW") then
                    DrinkName = strsub(L["CRYOLYSIS_DRINK_RANK"][StoneMaxRank[3]], 2, strlen(L["CRYOLYSIS_DRINK_RANK"][StoneMaxRank[3]]));
                elseif CryolysisConfig.CryolysisLanguage == "frFR" then
                    DrinkName = L["CRYOLYSIS_DRINK_RANK"][StoneMaxRank[3]] .. L["CRYOLYSIS_ITEM"].Provision;
                else
                    DrinkName = L["CRYOLYSIS_ITEM"].Provision .. L["CRYOLYSIS_DRINK_RANK"][StoneMaxRank[3]];
                end
            ...
            if itemName ~= "nil" then
                ...
                elseif itemName == L["CRYOLYSIS_ITEM"].ArcanePowder then Count.ArcanePowder = Count.ArcanePowder + ItemCount;
                elseif itemName == L["CRYOLYSIS_ITEM"].LightFeather then Count.LightFeather = Count.LightFeather + ItemCount;
                elseif itemName == L["CRYOLYSIS_ITEM"].RuneOfTeleportation then Count.RuneOfTeleportation = Count.RuneOfTeleportation + ItemCount;
                elseif itemName == L["CRYOLYSIS_ITEM"].RuneOfPortals then Count.RuneOfPortals = Count.RuneOfPortals + ItemCount;
                end
                -- Same thing for Mana Gem
                if string.find(itemName, L["CRYOLYSIS_ITEM"].Manastone) then
                    for i = 1, StoneMaxRank[2], 1 do
                        if string.find(itemName, L["CRYOLYSIS_ITEM"].Manastone .. CRYOLYSIS_STONE_RANK[i]) then
                ...
                -- Hearthstone
                if HearthstoneLocation == { container, slot } and itemName ~= L["CRYOLYSIS_ITEM"].Hearthstone
                        or HearthstoneOnHand == false then
                    if string.find(itemName, L["CRYOLYSIS_ITEM"].Hearthstone) then
    ...
    if (Provision > provision and Provision == GetContainerNumSlots(CryolysisConfig.ProvisionContainer)) then
        if (ProvisionsDestroy) then
            Cryolysis_Msg(L["CRYOLYSIS_MESSAGE"].Bag.FullPrefix .. GetBagName(CryolysisConfig.SoulshardContainer) .. L["CRYOLYSIS_MESSAGE"].Bag.FullDestroySuffix);
        else
            Cryolysis_Msg(L["CRYOLYSIS_MESSAGE"].Bag.FullPrefix .. GetBagName(CryolysisConfig.SoulshardContainer) .. L["CRYOLYSIS_MESSAGE"].Bag.FullSuffix);
        end
    end
end
...
function Cryolysis_FindSlot(ProvisionIndex, ProvisionSlot, itemInfo)
    ...
    for slot = 1, GetContainerNumSlots(CryolysisConfig.ProvisionContainer), 1 do -- go through every slot in the provision bag
        ...
        if string.find(foodstuff, Count.FoodLastName) ~= nil and StackCount < 20 and foodstuff == itemInfo
                ...
                or string.find(foodstuff, L["CRYOLYSIS_ITEM"].Provision) == nil then -- If that item isn't food or drink
            PickupContainerItem(ProvisionIndex, ProvisionSlot); -- Pick up the food/drink in the other bag
            PickupContainerItem(CryolysisConfig.ProvisionContainer, slot); -- Pick up self item, effectively swapping
...
function Cryolysis_SpellSetup()
    ...
    local StoneType = { L["CRYOLYSIS_ITEM"].Evocation, L["CRYOLYSIS_ITEM"].Manastone, L["CRYOLYSIS_ITEM"].Drink, L["CRYOLYSIS_ITEM"].Food };
    ...
    while true do
        ...
        if subSpellName then
            if (string.find(subSpellName, L["CRYOLYSIS_TRANSLATION"].Rank)) then
                local found = false;
                local _, _, rank = string.find(subSpellName, L["CRYOLYSIS_TRANSLATION"].Rank .. " (.+)");
                rank = tonumber(rank);
        ...
        for i = 1, table.getn(L["CRYOLYSIS_MANASTONE_NAMES"]), 1 do
            if spellName == L["CRYOLYSIS_MANASTONE_NAMES"][i] then
    ...
    for stoneID = 1, table.getn(StoneType), 1 do
        if StoneMaxRank[stoneID] ~= 0 then
            table.insert(L["CRYOLYSIS_SPELL_TABLE"], {
            ...
            StoneIDInSpellTable[stoneID] = table.getn(L["CRYOLYSIS_SPELL_TABLE"]);
        end
    end
    -- Updates the list of the spells with the new ranks
    for spell = 1, table.getn(L["CRYOLYSIS_SPELL_TABLE"]), 1 do
        for index = 1, table.getn(CurrentSpells.Name), 1 do
            if (L["CRYOLYSIS_SPELL_TABLE"][spell].Name == CurrentSpells.Name[index])
                    and L["CRYOLYSIS_SPELL_TABLE"][spell].ID ~= StoneIDInSpellTable[3] then
                L["CRYOLYSIS_SPELL_TABLE"][spell].ID = CurrentSpells.ID[index];
                L["CRYOLYSIS_SPELL_TABLE"][spell].Rank = CurrentSpells.subName[index];
            end
        end
    end

    -- Updates the duration of each spell according to its rank
    for index = 1, table.getn(L["CRYOLYSIS_SPELL_TABLE"]), 1 do
        if (index == 26) then -- For Polymorph
            if L["CRYOLYSIS_SPELL_TABLE"][index].ID ~= nil then
                L["CRYOLYSIS_SPELL_TABLE"][index].Length = L["CRYOLYSIS_SPELL_TABLE"][index].Rank * 10 + 10;
            end
        end
        if (index == 21) then -- For Frost bolt
            nameTalent, icon, tier, column, currRank, maxRank = GetTalentInfo(3, 7);
            if L["CRYOLYSIS_SPELL_TABLE"][index].ID ~= nil and L["CRYOLYSIS_SPELL_TABLE"][index].Rank <= 7 then
                L["CRYOLYSIS_SPELL_TABLE"][index].Length = ceil(4.5 + currRank + (L["CRYOLYSIS_SPELL_TABLE"][index].Rank * 0.5));
            else
                L["CRYOLYSIS_SPELL_TABLE"][index].Length = L["CRYOLYSIS_SPELL_TABLE"][index].Length + currRank
            end
        end
        if (index == 16) then -- for Fireball
            if L["CRYOLYSIS_SPELL_TABLE"][index].ID ~= nil and L["CRYOLYSIS_SPELL_TABLE"][index].Rank <= 3 then
                L["CRYOLYSIS_SPELL_TABLE"][index].Length = 6;
            end
        end
        if (index == 14) then -- For fireblast
            if L["CRYOLYSIS_SPELL_TABLE"][index].ID ~= nil then
                _, _, _, _, currRank, maxRank = GetTalentInfo(2, 7);
                L["CRYOLYSIS_SPELL_TABLE"][index].Length = 8 - (currRank * 0.5);
            end
        end
        if (index == 19) then -- For frost nova
            if L["CRYOLYSIS_SPELL_TABLE"][index].ID ~= nil then
                _, _, _, _, currRank, maxRank = GetTalentInfo(3, 6);
                L["CRYOLYSIS_SPELL_TABLE"][index].Length = 25 - (currRank * 2);
            end
        end
    end

    for spellID = 1, MAX_SPELLS, 1 do
        local spellName, subSpellName = GetSpellBookItemName(spellID, "spell");
        if (spellName) then
            for index = 1, table.getn(L["CRYOLYSIS_SPELL_TABLE"]), 1 do
                if L["CRYOLYSIS_SPELL_TABLE"][index].Name == spellName then
                    Cryolysis_MoneyToggle();
                    CryolysisTooltip:SetSpellBookItem(spellID, 1);
                    local _, _, ManaCost = string.find(CryolysisTooltipTextLeft2:GetText(), "(%d+)");
                    if not L["CRYOLYSIS_SPELL_TABLE"][index].ID then
                        L["CRYOLYSIS_SPELL_TABLE"][index].ID = spellID;
                    end
                    L["CRYOLYSIS_SPELL_TABLE"][index].Mana = tonumber(ManaCost);
                end
            end
        end
    end
    --	if L["CRYOLYSIS_SPELL_TABLE"][1].ID or L["CRYOLYSIS_SPELL_TABLE"][2].ID then
    --		MountAvailable = true;
    --	else
    --		MountAvailable = false;
    --	end
    --
    if L["CRYOLYSIS_SPELL_TABLE"][10].ID ~= nil then
        StoneMaxRank[4] = L["CRYOLYSIS_SPELL_TABLE"][10].Rank
        StoneIDInSpellTable[4] = L["CRYOLYSIS_SPELL_TABLE"][10].ID
    end
    if L["CRYOLYSIS_SPELL_TABLE"][11].ID ~= nil then
        StoneMaxRank[3] = L["CRYOLYSIS_SPELL_TABLE"][11].Rank
        StoneIDInSpellTable[3] = L["CRYOLYSIS_SPELL_TABLE"][11].ID
    end
    if L["CRYOLYSIS_SPELL_TABLE"][49].ID ~= nil then
        StoneMaxRank[1] = L["CRYOLYSIS_SPELL_TABLE"][49].Rank
        StoneIDInSpellTable[1] = L["CRYOLYSIS_SPELL_TABLE"][49].ID
    end
end
...
function Cryolysis_FindSpellAttribute(type, attribute, array)
    for index = 1, table.getn(L["CRYOLYSIS_SPELL_TABLE"]), 1 do
        if string.find(L["CRYOLYSIS_SPELL_TABLE"][index][type], attribute) then return L["CRYOLYSIS_SPELL_TABLE"][index][array]; end
    end
    return nil;
end
...
function Cryolysis_ColdBlock()
    local start, duration = GetSpellCooldown(L["CRYOLYSIS_SPELL_TABLE"][41].ID, BOOKTYPE_SPELL);
    ...
    if (cooldown < 1.5) or (cooldown < 298 and cooldown > 290) then
        SpellStopCasting();
        CastSpell(L["CRYOLYSIS_SPELL_TABLE"][41].ID, "spell");
    else
        CastSpell(L["CRYOLYSIS_SPELL_TABLE"][42].ID, "spell");
    end
end
...
function Nethaera_FrostyBolt(Clearcast, PI)
    if Clearcast then
        SpellStopCasting();
        CastSpell(L["CRYOLYSIS_SPELL_TABLE"][5].ID, "spell");
...
function Cryolysis_Decursive()
    ...
    if cursed ~= nil then
        ...
        CastSpell(L["CRYOLYSIS_SPELL_TABLE"][33].ID, "spell");
...
function Cryolysis_Metamorph()
    ...
    for i = 1, table.getn(morphs), 1 do
        if L["CRYOLYSIS_SPELL_TABLE"][morphs[i]].ID then
            spells = spells + 1;
            availableMorphs[spells] = morphs[i];
        end
    end
    CastSpell(L["CRYOLYSIS_SPELL_TABLE"][availableMorphs[random(1, table.getn(availableMorphs))]].ID, "spell");
end
...
function Cryolysis_UseItem(type, button, keybind)
    ...
    if type == "Hearthstone" and button == "RightButton" then
        if (HearthstoneOnHand) then
            ...
            Cryolysis_Msg(L["CRYOLYSIS_MESSAGE"].Error.NoHearthStone, "USER");
        end
        -- If you click on  the Mana gem :
    elseif type == "Manastone" and button == "LeftButton" then
        if (UnitPower("player") == UnitPowerMax("player")) then -- Max mana! Not going to waste it
            Cryolysis_Msg(L["CRYOLYSIS_MESSAGE"].Error.FullMana, "USER");
        elseif CryolysisConfig.ManaStoneOrder == 2 then
            for i = StoneMaxRank[2], 1, -1 do
                ...
                if Manastone.OnHand[i] then
                    local start, duration, enable = GetContainerItemCooldown(Manastone.Location[i][1], Manastone.Location[i][2]);
                    if start > 0 and duration > 0 then
                        Cryolysis_Msg(L["CRYOLYSIS_MESSAGE"].Error.ManaStoneCooldown, "USER");
                        break;
                    else
                        SpellStopCasting();
        ...
        if (UnitExists("target") and UnitIsPlayer("target") and UnitIsFriend("player", "target") and UnitName("target") ~= UnitName("player")) then
            for i = StoneMaxRank[4], 1, -1 do
                ...
                if UnitLevel("target") >= var then
                    CastSpellByName(L["CRYOLYSIS_SPELL_TABLE"][10].Name .. "(" .. L["CRYOLYSIS_TRANSLATION"].Rank .. " " .. i .. ")");
                    FoodLastRank = i;
                    if GetLocale() == "frFR" then
                        Count.FoodLastName = L["CRYOLYSIS_FOOD_RANK"][i] .. L["CRYOLYSIS_ITEM"].Provision;
                    else
                        Count.FoodLastName = L["CRYOLYSIS_ITEM"].Provision .. L["CRYOLYSIS_FOOD_RANK"][i];
                    end
                    break;
                end
            end
            ...
            if GetLocale() == "frFR" then
                Count.FoodLastName = L["CRYOLYSIS_FOOD_RANK"][StoneMaxRank[4]] .. L["CRYOLYSIS_ITEM"].Provision;
            else
                Count.FoodLastName = L["CRYOLYSIS_ITEM"].Provision .. L["CRYOLYSIS_FOOD_RANK"][StoneMaxRank[4]];
            end
        end
    elseif type == "Food" then
    ...
    elseif type == "Drink" and button == "RightButton" then
        if (UnitExists("target") and UnitIsPlayer("target") and UnitIsFriend("player", "target") and UnitName("target") ~= UnitName("player")) then
            for i = StoneMaxRank[3], 1, -1 do
                local var = i * 10 - 15
                if UnitLevel("target") >= var then
                    CastSpellByName(L["CRYOLYSIS_SPELL_TABLE"][11].Name .. "(" .. L["CRYOLYSIS_TRANSLATION"].Rank .. " " .. i .. ")");
                    DrinkLastRank = i;
                    if GetLocale() == "frFR" then
                        Count.DrinkLastName = L["CRYOLYSIS_DRINK_RANK"][i] .. L["CRYOLYSIS_ITEM"].Provision;
                    else
                        Count.DrinkLastName = L["CRYOLYSIS_ITEM"].Provision .. L["CRYOLYSIS_DRINK_RANK"][i];
                    end
                    break;
                end
            end
            ...
            if GetLocale() == "frFR" then
                Count.DrinkLastName = L["CRYOLYSIS_DRINK_RANK"][StoneMaxRank[3]] .. L["CRYOLYSIS_ITEM"].Provision;
            else
                Count.DrinkLastName = L["CRYOLYSIS_ITEM"].Provision .. L["CRYOLYSIS_DRINK_RANK"][StoneMaxRank[3]];
            end
        end
    elseif type == "Drink" then
        if button == "MiddleButton" or Ctrl then
    ...
    elseif type == "Mount" and button == "LeftButton" then
        ...
        if Mount.Available then
            ...
            Cryolysis_Msg(L["CRYOLYSIS_MESSAGE"].Error.NoRiding, "USER");
        end
...
function Cryolysis_CreateMenu()
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][38].ID then
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][40].ID then
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][39].ID then
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][47].ID then
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][31].ID then
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][30].ID then
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][37].ID then
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][51].ID then
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][46].ID then
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][28].ID then
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][29].ID then
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][27].ID then
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][22].ID or L["CRYOLYSIS_SPELL_TABLE"][18].ID then
        menuVariable = _G["CryolysisBuffMenu1"];
        menuVariable:ClearAllPoints();
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][4].ID then
        menuVariable = _G["CryolysisBuffMenu2"];
        menuVariable:ClearAllPoints();
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][13].ID then
        menuVariable = _G["CryolysisBuffMenu3"];
        menuVariable:ClearAllPoints();
        if BuffButtonPosition == 0 then
            menuVariable:SetPoint("CENTER", "CryolysisBuffMenuButton", "CENTER", 3000, 3000);
            menuVariable:SetScale(CryolysisConfig.CryolysisStoneScale / 100);
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][25].ID or L["CRYOLYSIS_SPELL_TABLE"][23].ID then
        menuVariable = _G["CryolysisBuffMenu4"];
        menuVariable:ClearAllPoints();
        if BuffButtonPosition == 0 then
            menuVariable:SetPoint("CENTER", "CryolysisBuffMenuButton", "CENTER", 3000, 3000);
            menuVariable:SetScale(CryolysisConfig.CryolysisStoneScale / 100);
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][15].ID then
        menuVariable = _G["CryolysisBuffMenu5"];
        menuVariable:ClearAllPoints();
        if BuffButtonPosition == 0 then
            menuVariable:SetPoint("CENTER", "CryolysisBuffMenuButton", "CENTER", 3000, 3000);
            menuVariable:SetScale(CryolysisConfig.CryolysisStoneScale / 100);
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][50].ID then
        menuVariable = _G["CryolysisBuffMenu6"];
        menuVariable:ClearAllPoints();
        if BuffButtonPosition == 0 then
            menuVariable:SetPoint("CENTER", "CryolysisBuffMenuButton", "CENTER", 3000, 3000);
            menuVariable:SetScale(CryolysisConfig.CryolysisStoneScale / 100);
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][33].ID then
        menuVariable = _G["CryolysisBuffMenu7"];
        menuVariable:ClearAllPoints();
        if BuffButtonPosition == 0 then
            menuVariable:SetPoint("CENTER", "CryolysisBuffMenuButton", "CENTER", 3000, 3000);
            menuVariable:SetScale(CryolysisConfig.CryolysisStoneScale / 100);
    ...
    if L["CRYOLYSIS_SPELL_TABLE"][35].ID then
        menuVariable = _G["CryolysisBuffMenu8"];
        menuVariable:ClearAllPoints();
        if BuffButtonPosition == 0 then
            menuVariable:SetPoint("CENTER", "CryolysisBuffMenuButton", "CENTER", 3000, 3000);
            menuVariable:SetScale(CryolysisConfig.CryolysisStoneScale / 100);
...
function Cryolysis_BuffCast(type, click, nosave)
    ...
    if not L["CRYOLYSIS_SPELL_TABLE"][type].ID and type ~= 23 then
        CastSpell(L["CRYOLYSIS_SPELL_TABLE"][18].ID, "spell");
    elseif click == "RightButton" then
        -- Ice armor or Mage armor
        if type == 22 and L["CRYOLYSIS_SPELL_TABLE"][24].ID ~= nil then
            CastSpell(L["CRYOLYSIS_SPELL_TABLE"][24].ID, "spell");
            if not nosave then CryolysisPrivate.LastBuff = 24; end
            -- Arcane Intellect or Arcane Brilliance
        elseif type == 4 and L["CRYOLYSIS_SPELL_TABLE"][2].ID ~= nil then
            CastSpell(L["CRYOLYSIS_SPELL_TABLE"][2].ID, "spell");
            if not nosave then CryolysisPrivate.LastBuff = 2; end
            -- Dampen or Amplify Magic
        elseif type == 13 and L["CRYOLYSIS_SPELL_TABLE"][1].ID ~= nil then
            CastSpell(L["CRYOLYSIS_SPELL_TABLE"][1].ID, "spell");
            if not nosave then CryolysisPrivate.LastBuff = 1; end
            -- Ice barrier or mana shield
        elseif type == 23 and L["CRYOLYSIS_SPELL_TABLE"][25].ID ~= nil then
            CastSpell(L["CRYOLYSIS_SPELL_TABLE"][25].ID, "spell");
            if not nosave then CryolysisPrivate.LastBuff = 25; end
            -- Fire Ward or Frost Ward
        elseif type == 15 and L["CRYOLYSIS_SPELL_TABLE"][20].ID ~= nil then
            CastSpell(L["CRYOLYSIS_SPELL_TABLE"][20].ID, "spell");
            if not nosave then CryolysisPrivate.LastBuff = 20; end
        elseif type == 33 then
            Cryolysis_Decursive();
            if not nosave then CryolysisPrivate.Lastbuff = 33; end
        else
            CastSpell(L["CRYOLYSIS_SPELL_TABLE"][type].ID, "spell");
            if not nosave then CryolysisPrivate.LastBuff = type; end
        end
    else
        if type == 4 then
            for i = L["CRYOLYSIS_SPELL_TABLE"][4].Rank, 1, -1 do
                local MinLev = (i * 14) - 24
                if UnitLevel("target") >= MinLev then
                    CastSpellByName(L["CRYOLYSIS_SPELL_TABLE"][4].Name .. "(" .. L["CRYOLYSIS_TRANSLATION"].Rank .. " " .. i .. ")");
                    break;
                end
            end
        elseif type == 33 then
        ...
        elseif type == 23 then
            if L["CRYOLYSIS_SPELL_TABLE"][23].ID then
                CastSpell(L["CRYOLYSIS_SPELL_TABLE"][23].ID, "spell");
            elseif L["CRYOLYSIS_SPELL_TABLE"][25].ID then
                CastSpell(L["CRYOLYSIS_SPELL_TABLE"][25].ID, "spell");
            else
                CastSpell(L["CRYOLYSIS_SPELL_TABLE"][18].ID, "spell");
            end
...
function Cryolysis_PortalCast(type, click)
    if type <= 6 and Count.RuneOfTeleportation == 0 then
        Cryolysis_Msg(L["CRYOLYSIS_MESSAGE"].Error.RuneOfTeleportationNotPresent, "USER");
        return;
    elseif type >= 7 and Count.RuneOfPortals == 0 then
        Cryolysis_Msg(L["CRYOLYSIS_MESSAGE"].Error.RuneOfPortalsNotPresent, "USER");
        return;
    end
    CryolysisPrivate.LastPortal = type;
    if click == "RightButton" then CryolysisPrivate.ChatSilence = true; end
    CastSpell(L["CRYOLYSIS_SPELL_TABLE"][PortalTempID[type]].ID, "spell");
    CryolysisPrivate.AlphaPortalMenu = 1;
    CryolysisPrivate.AlphaPortalVar = GetTime() + 3;
end
...
function CryolysisGeneralTab_OnClick(id)
    ...
    if id == 1 then
        ...
        CryolysisGeneralPageText:SetText(L["CRYOLYSIS_CONFIGURATION"].Menu1);
    elseif id == 2 then
        ...
        HideUIPanel(CryolysisGraphOptionMenu);
        CryolysisGeneralIcon:SetTexture("Interface\\QuestFrame\\UI-QuestLog-BookIcon");
        CryolysisGeneralPageText:SetText(L["CRYOLYSIS_CONFIGURATION"].Menu2);
    elseif id == 3 then
        Reorder.UpdateOrder()
        HideUIPanel(CryolysisProvisionMenu);
        HideUIPanel(CryolysisMessageMenu);
        ...
        HideUIPanel(CryolysisGraphOptionMenu);
        CryolysisGeneralIcon:SetTexture("Interface\\QuestFrame\\UI-QuestLog-BookIcon");
        CryolysisGeneralPageText:SetText(L["CRYOLYSIS_CONFIGURATION"].Menu3);
    elseif id == 4 then
        HideUIPanel(CryolysisProvisionMenu);
        HideUIPanel(CryolysisMessageMenu);
        HideUIPanel(CryolysisButtonMenu);
        ...
        HideUIPanel(CryolysisGraphOptionMenu);
        CryolysisGeneralIcon:SetTexture("Interface\\QuestFrame\\UI-QuestLog-BookIcon");
        CryolysisGeneralPageText:SetText(L["CRYOLYSIS_CONFIGURATION"].Menu4);
    elseif id == 5 then
        HideUIPanel(CryolysisProvisionMenu);
        HideUIPanel(CryolysisMessageMenu);
        HideUIPanel(CryolysisButtonMenu);
        ...
        CryolysisGeneralIcon:SetTexture("Interface\\QuestFrame\\UI-QuestLog-BookIcon");
        CryolysisGeneralPageText:SetText(L["CRYOLYSIS_CONFIGURATION"].Menu5);
    end
end
...
Cryolysis_Hook = function(orig, new, type)
    ...
    if (not Hx_Hooks[orig]) then
        Hx_Hooks[orig] = {}; Hx_Hooks[orig].before = {}; Hx_Hooks[orig].before.n = 0; Hx_Hooks[orig].after = {}; Hx_Hooks[orig].after.n = 0; Hx_Hooks[orig].hide = {}; Hx_Hooks[orig].hide.n = 0; Hx_Hooks[orig].replace = {}; Hx_Hooks[orig].replace.n = 0; Hx_Hooks[orig].orig = _G[orig];
    else
        for key, value in Hx_Hooks[orig][type] do if (value == _G[new]) then return; end end
    end
    Cryolysis_Push(Hx_Hooks[orig][type], _G[new]);
    _G[orig] = function(...) Cryolysis_HookHandler(orig, pack(...)); end;
end
...
function Cryolysis_CastSpell(spellId, spellbookTabNum)
    print("Cast spell begin");
    local Name, Rank = GetSpellBookItemName(spellId, spellbookTabNum);
    if Rank ~= nil then
    ...
    print("Cast spell end");
end

function Cryolysis_CastSpellByName(Spell)
    print("Cryolysis_CastSpellByName begin");
    ...
    print("Cryolysis_CastSpellByName end");
end
